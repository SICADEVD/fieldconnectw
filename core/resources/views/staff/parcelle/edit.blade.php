...
@section('panel')
    <div class="row mb-none-30">
        <div class="col-lg-12 mb-30">
            <div class="card">
                <div class="card-body"> 
         {!! Form::model($parcelle, ['method' => 'POST','route' => ['staff.traca.parcelle.store', $parcelle->id],'class'=>'form-horizontal', 'id'=>'flocal', 'enctype'=>'multipart/form-data']) !!}
                        ...
                        <div class="form-group row">
                                ...
                                <select class="form-control" name="localite" id="localite" required>
                                    ...
                                    @foreach($localites as $localite)
                                        <option value="{{ $localite->id }}" @selected($localite->id==$parcelle->producteur->localite->id)>
                                            {{ $localite->nom }}</option>
                                    @endforeach
                                </select>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <select class="form-control" name="producteur" id="producteur" required>
                                    ...
                                    @foreach($producteurs as $producteur)
                                        <option value="{{ $producteur->id }}" data-chained="{{ $producteur->localite->id }}" @selected($producteur->id==$parcelle->producteur_id)>
            ...
            <div class="col-xs-12 col-sm-8">
                <input type="text" name="culture" placeholder="Café, Cacao" class="form-control @error('culture') is-invalid @enderror" value="{{ old('culture') ?? $parcelle->culture }}" required>
        </div>
    </div>
    <div class="form-group row">
 ...
 <tr>
    ...
    <div class="col-xs-12 col-sm-12">
        <div class="form-group row">
            ...
            {!! Form::number('anneeCreation', old('anneeCreation') ?? $parcelle->anneeCreation, array('placeholder' => __('Année de création'),'class' => 'form-control text4','id'=>'anneeCreation-1' ,'required','min'=>1990, 'max'=>gmdate('Y')-5)) !!}
        </div>
    </div>
...
@push('breadcrumb-plugins')
    <x-back route="{{ route('staff.traca.parcelle.index') }}" />
@endpush
